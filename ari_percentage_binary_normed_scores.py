...
from ari_scores_binary_percentage_normed import ari_scores_binary_percentage_normed
...
distance_names=['dice', 'hamming', 'jaccard','kulsinski','rogerstanimoto','russellrao','sokalmichener','sokalsneath','yule']

data_file_names =  sorted(os.listdir(path_data))
label_file_names =  sorted(os.listdir(path_label))

scores = []
data_book = xlsxwriter.Workbook("/home/milica/Desktop/po/projekat_po1/Affy/ARI/ARI_normed_binary_results.xlsx")
...
for i in range(0,len(data_file_names)):
    ...
    ari = ari_scores_binary_percentage_normed(np.array(data,dtype=float),label,data_file_names[i],0.1,0.05)
    scores.append(ari)
print('\n')
print(scores)
...
pio.write_image(fig, file='/home/milica/Desktop/po/projekat_po1/Affy/ARI/ari_percentage_bianry_normed_scores.png', format='png')
