...
def ari_scores_binary_percentage_normed(data,labels,file,p1,p2):
    ...
    ari_dice = metrics.adjusted_rand_score(lbls,dice_label)
 #radi normalno

    hamming_label = k_means(data, np.max(np.unique(labels)), hamming)
    ari_hamming = metrics.adjusted_rand_score(lbls,hamming_label)
#radi normalno

    jaccard_label = k_means(data, np.max(np.unique(labels)), jaccard)
    ari_jaccard = metrics.adjusted_rand_score(lbls,jaccard_label)
#radi normalno
    rogerstanimoto_label = k_means(data, np.max(np.unique(labels)), rogerstanimoto)
    ari_rogerstanimoto = metrics.adjusted_rand_score(lbls,rogerstanimoto_label)
#radi normalno
    russellrao_label = k_means(data, np.max(np.unique(labels)), russellrao)
    ari_russellrao = metrics.adjusted_rand_score(lbls,russellrao_label)
#radi normalno

    sokalmichener_label = k_means(data, np.max(np.unique(labels)), sokalmichener)
    ari_sokalmichener = metrics.adjusted_rand_score(lbls,sokalmichener_label)
#radi normalno

    sokalsneath_label = k_means(data, np.max(np.unique(labels)), sokalsneath)
    ari_sokalsneath = metrics.adjusted_rand_score(lbls,sokalsneath_label)
#radi normalno
    yule_label = k_means(data, np.max(np.unique(labels)), yule)
    ari_yule = metrics.adjusted_rand_score(lbls,yule_label)
#radi normalno


    return [file,ari_dice,ari_hamming,ari_jaccard,ari_rogerstanimoto,ari_russellrao,ari_russellrao,ari_sokalmichener,ari_sokalsneath,ari_yule]
